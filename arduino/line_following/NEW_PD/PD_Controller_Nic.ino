...
void loop()
{
  ...
  {   
    ...
    if(frontPollingValues[0] <= 500 && frontPollingValues[1] <= 500) // Determine if the front sensors are seeing white
	{
	  ...
	  {
	    stop_motors();
            drive_motor(RIGHTMOTOR, FWD, 30);
            drive_motor(LEFTMOTOR, FWD, 30);
            delay(250);
	    //Play Game
	    delay(4000);	
	    drive_motor(RIGHTMOTOR, BWD, 30); 		//Drive backwards for some set amount of time
    	    drive_motor(LEFTMOTOR, BWD, 30); 		//to get out of box
	    delay(1000);	
	    while((pollingValues[1] > 500)) 					//Drive backwards until line is detected
	    {
              poll.read(pollingValues);
	      drive_motor(RIGHTMOTOR, BWD, rightMotorSpeed); 
    	      drive_motor(LEFTMOTOR, BWD, leftMotorSpeed); 
	    }
	    if(lastTurn == RIGHT)	//Once line is detected turn the opposite direction from the last turn
            {
	      turn(LEFT);
	    }
	    else
  ...
  {   
	...
	//Play Game
	delay(2000);	
	drive_motor(RIGHTMOTOR, BWD, rightMotorSpeed); 		//Drive backwards for some set amount of time
    	drive_motor(LEFTMOTOR, BWD, leftMotorSpeed); 		//to get out of box
	delay(1000);	
	while((pollingValues[0] > 500)) 					//Drive backwards until line is detected
	{
	  drive_motor(RIGHTMOTOR, BWD, rightMotorSpeed); 
    	  drive_motor(LEFTMOTOR, BWD, leftMotorSpeed); 
	}
	if(lastTurn == LEFT)	//Once line is detected turn the opposite direction from the last turn
	{
	  turn(RIGHT);
	}
	else
